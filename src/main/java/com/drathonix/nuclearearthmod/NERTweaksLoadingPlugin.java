...
import com.drathonix.nertweaks.utils.GameResourceHelper;
...
public class NERTweaksLoadingPlugin implements IFMLLoadingPlugin
{
    public NERTweaksLoadingPlugin()
    {
        MixinBootstrap.init();
        attemptLoadMixin("industrialforegoing");
        attemptLoadMixin("nuclearcraft");
        attemptLoadMixin("techreborn");
        attemptLoadMixin("reborncore");
        MixinBootstrap.init();
        GameResourceHelper.clear();
    }
    ...
    private boolean attemptLoadMixin(String modid) {
        if(GameResourceHelper.load(modid)){
            Mixins.addConfiguration("mixin/mixins.nertweaks." + modid + ".json");
            return true;
        }
        return false;
    }
